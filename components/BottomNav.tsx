    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 safe-bottom" style={{ background: 'linear-gradient(to top, #0c1520, #1A2535)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="max-w-lg mx-auto flex">
        {TABS.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={`flex-1 flex flex-col items-center py-2.5 gap-0.5 transition-all ${active ? 'text-palace-red' : 'text-white/35 hover:text-white/60'}`}>
