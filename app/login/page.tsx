...
export default function LoginPage() {
  ...
  async function handleLogin() {
    ...
    const { data } = await supabase.from('users').select('*').eq('display_name', selected.name).single();
    setLoading(false);
    if (!data || data.pin_hash !== hashed) {
      setError('Incorrect PIN'); setPin('');
      setTimeout(() => setError(''), 2000); return;
    }
  ...
  if (selected) {
    return (
      <div className="fixed inset-0 bg-bg-base flex flex-col items-center justify-center p-6 z-50">
        <button onClick={() => { setSelected(null); setPin(''); setError(''); }} className="absolute top-6 left-6 text-white/40 hover:text-white text-sm flex items-center gap-1 transition-colors">
          ← Back
        </button>
        <div className="w-24 h-24 rounded-2xl flex items-center justify-center mb-5 shadow-2xl" style={{ backgroundColor: selected.colour }}>
        ...
        <button onClick={handleLogin} disabled={loading || pin.length < 4}
          ...
          {loading ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Enter →'}
        </button>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 flex flex-col overflow-hidden" style={{ background: 'radial-gradient(ellipse at 50% 0%, #3d0a14 0%, #0F1923 60%)' }}>
      <div className="flex flex-col items-center pt-12 pb-6 px-6 flex-shrink-0">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="font-display font-bold text-3xl text-white leading-none tracking-wide">CPFC<span className="text-gold">Mundial</span></h1>
        ...
        <div className="grid grid-cols-2 gap-3">
          {PLAYERS.map(p => (
            <button key={p.name} onClick={() => setSelected(p)}
              ...
              <div className="w-14 h-14 rounded-xl flex items-center justify-center shadow-lg font-display font-bold text-2xl text-white" style={{ backgroundColor: p.colour }}>
                {initials(p.name)}
              </div>
